...
/**
 * Видит ли компания файлы задания — то, что приложил к заказу клиент
 * (решение пользователя от 5 сентября 2026).
 *
 * Исполнителю они видны всегда, остальным — пока заказ принимает предложения:
 * чертежи и планы клиент прикладывает именно для того, чтобы по ним посчитали
 * цену, и требовать предложения вслепую значит работать против сути продукта
 * (ТЗ §1). Как только исполнитель выбран, у остальных доступ пропадает.
 *
 * Сдачи компании этим правилом не открываются никогда: они остаются только
 * сторонам сделки (ТЗ §4.1).
 *
 * Правило одно на все три места, где оно нужно: состав `OrderDetail.files`,
 * выдача signed URL и текст под пустым списком в интерфейсе.
 */
export function companySeesTaskFiles(
  orderStatus: OrderStatus,
  /** Статус предложения этой компании. `null` — предложения нет. */
  offerStatus: OfferStatus | null,
): boolean {
  if (offerStatus !== null && isExecutorOffer(offerStatus)) return true;

  return acceptsOffers(orderStatus);
}
