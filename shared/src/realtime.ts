 ...
 *
 * Адресат у таких событий всегда именной — клиент заказа и компания-автор.
 * В комнату заказа они не уходят: там сидят и другие компании с активным
 * предложением, а чужие предложения им не показывают (ТЗ §4.1).
 */
...
export interface SubscribeAck {
  ...
  error?: string;
  /**
   * Имеет ли смысл повторить попытку.
   *
   * Отказы бывают двух сортов, и клиенту надо их различать. «Заказ не найден»
   * повтором не исправишь — правило не изменится. А недоступная база или
   * упёршийся лимит частоты пройдут сами, и без повтора вкладка осталась бы
   * вне комнаты навсегда: сокет жив, переподключения не будет, и событий
   * страница просто не увидит.
   */
  retryable?: boolean;
}
