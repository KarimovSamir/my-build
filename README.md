...
npm run test:e2e     # e2e на живой Supabase из backend/.env, в конце — db:reset
npm run db:reset     # очистить базу и заново залить демо-данные
```

e2e работают с той же базой, что указана в `backend/.env`, и после прогона
возвращают её к стандартному виду: удаляется всё, кроме четырёх демо-аккаунтов,
включая все остальные учётки и все файлы в бакете, а демо-данные заливаются
заново. На базе с настоящими пользователями e2e не запускать.
