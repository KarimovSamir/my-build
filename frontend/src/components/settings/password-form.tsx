 ...
 *
 * Текущий пароль спрашивается не для галочки. Сам `updateUser` меняет пароль
 * по одной живой сессии, поэтому база принимает смену только вскоре после
 * входа (триггер `on_auth_user_password_reauth`, `PASSWORD_CHANGE_WINDOW_MINUTES`):
 * угнанная сессия без пароля свежий вход не получит. Форма входит тем же email
 * и паролем прямо перед сменой — пользователь тот же, сессия просто обновляется
 * и попадает в окно.
 */
