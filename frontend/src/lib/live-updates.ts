...
/**
 * Раздел «Документы» (ТЗ §7): список пополняется, когда исполнитель загружает
 * файлы сдачи.
 *
 * Событие приходит клиенту в личную комнату (ТЗ §8), поэтому вживую список
 * догоняет только у него — и это не упущение, а вся правда о разделе: новые
 * файлы по заказу появляются лишь от исполнителя, а его собственной вкладке
 * событие о своём же действии не адресуется. Задание клиента прикладывается
