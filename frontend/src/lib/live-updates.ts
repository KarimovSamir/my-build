 ...
 * ушедший из ленты (выбран исполнитель или заказ удалён) — `order:status_changed`
 * туда же. Движение внутри ленты (первое предложение) в неё не приходит:
 * строка от него не меняется, а всем компаниям это рассказало бы о конкуренте.
 *
 * `notification:created` — ради своего предложения: принятое или отклонённое,
 * оно меняет, стоит ли заказ в ленте этой компании.
 */
