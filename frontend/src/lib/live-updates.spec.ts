...
describe("состав событий", () => {
  ...
  /**
   * Документы меняются от загрузки файлов и от принятия предложения: во втором
   * случае компания становится стороной сделки, и файлы заказа появляются
   * в её разделе. Статус заказа состав файлов не меняет, а `notification:created`
   * заставил бы раздел перечитываться на каждое движение по любому заказу.
   */
  it("документы слушают файлы и смену статуса предложения", () => {
    expect(DOCUMENTS_EVENTS).toEqual([
