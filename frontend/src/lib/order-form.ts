 ...
 *
 * Двух файлов с одинаковым именем быть не может: в списке они выглядели бы
 * одной строкой, и какой из них какой — не понять ни в форме, ни потом
 * в заказе. Это касается и того же файла, выбранного второй раз: молча его
 * пропускать значило бы съесть выбор пользователя без объяснения.
 *
 * `taken` — имена, уже приложенные там же, куда уйдёт загрузка (файлы открытой
 * сдачи). Без них правило действовало бы только внутри одной отправки, и двумя
 * заходами в ту же сдачу клали бы две неразличимые строки.
 *
...
export function addFiles(
  ...
  for (const file of incoming) {
    ...
    if (takenNames.has(file.name.toLowerCase())) {
      rejected.push(
        `«${file.name}» уже приложен — переименуйте файл или загрузите другой`,
      );
      continue;
    }
