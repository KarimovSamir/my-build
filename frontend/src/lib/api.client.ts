 ...
 *
 * Зеркало `api.server.ts`: тот берёт токен из cookie сессии на сервере, этот —
 * из той же сессии Supabase в браузере. Вызывающий код о токене не знает и забыть его
 * не может.
 *
