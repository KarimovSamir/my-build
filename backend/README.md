...
Unit-тесты базы не требуют. e2e работают с настоящей базой из `.env` и
заводят в ней своих пользователей и заказы. После прогона база очищается
целиком и получает стандартные демо-данные — это делает сам прогон.

```bash
npm run db:reset      # очистить базу и залить демо-данные (после ручных проверок)
npm run db:wipe       # только очистить: остаются четыре демо-учётки без данных
npm run db:seed       # только залить демо-данные
```

Очистка удаляет всё, кроме четырёх демо-учёток: и учётки, которые завели
посетители, и все файлы в бакете.
