 ...
 *
 * Счётчики живут в памяти процесса — при нескольких экземплярах backend'а
 * лимит окажется общим только внутри каждого из них. На Render Free инстанс
 * один, и это записано как условие деплоя; масштабирование вширь потребует
 * общего хранилища окон.
 *
 * Ключом для анонимных запросов служит IP; настоящий адрес за обратным прокси
 * даёт `app.set('trust proxy', 1)` в `bootstrap.ts`.
 */
