 ...
 *
 * `id` третьим ключом в индекс не входит и войти не должен — он там только
 * ради устойчивости выдачи. Совпадение `createdAt` здесь обычное дело:
 * `createManyAndReturn` ставит всей пачке уведомлений одного перехода время
 * начала транзакции, одно на все строки. Без уникального ключа порядок внутри
 * такой пачки Postgres волен менять между запросами, а в постраничном списке
 * это уже не косметика — строка способна пропасть со второй страницы.
 */
