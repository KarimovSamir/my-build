 ...
 *
 * И только с подтверждённым email. Профиль появляется при регистрации, до
 * подтверждения адреса, и без этого условия «компания», заведённая на чужой
 * адрес, сразу показывалась бы клиентам вместе с контактами. Время
 * подтверждения копирует в профиль триггер из auth.users.
 *
