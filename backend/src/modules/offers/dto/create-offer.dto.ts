...
export class CreateOfferDto {
  // Версия не проверяется — ровно по тем же причинам, что и в `common/uuid.ts`:
  // задача проверки в том, чтобы в Postgres не уехало то, что он не приведёт
  // к типу `uuid`. Требование «версия 4» отвергало бы валидный UUIDv7, то есть
  // смена генератора идентификаторов сломала бы один этот маршрут.
  @IsUUID('all', { message: 'Некорректный заказ' })
  orderId!: string;
